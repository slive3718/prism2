        ...
        <table class="table table-bordered table-sm" id="abstractTable">
            ...
            <?php if (!empty($abstracts)): ?>
                <?php foreach ($abstracts as $abstract):
                    ...
                    ?>
                    <tr style="<?= $display ?>">
                        <td><input type="checkbox" class="row-select" data-abstract-id="<?= $abstract['abstract_id'] ?>"></td>
                        <td><?= $authorsString ?></td>
                        <td><?= htmlspecialchars($abstract['details']->custom_id) ?></td>
                        <td><?= htmlspecialchars(strip_tags($abstract['details']->title)) ?></td>
            ...
            <?php foreach ($panels as $panel): ?>
                <tr>
                    ...
                    <td></td>
                    <td>
                        Panel #<?= htmlspecialchars($panel['custom_id']) ?>
                        <ol>
                            <?php foreach ($panel['panel_subs'] as $subPaper): ?>
                                <li class="fw-normal">
                                    <span class="text-muted"><?= htmlspecialchars($subPaper['custom_id']) ?></span>
                                    <?= htmlspecialchars($subPaper['submitter']['name']) . ' ' . htmlspecialchars($subPaper['submitter']['surname']) ?>
                                    <?= $subPaper['individual_panel_title'] ? '(' . htmlspecialchars(strip_tags($subPaper['individual_panel_title'])) . ')' : '' ?>
                                </li>
                            <?php endforeach; ?>
                        </ol>
                    </td>
