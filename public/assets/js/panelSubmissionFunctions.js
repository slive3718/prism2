    ...
    $('.addMarkedPanel').on('click', function(){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: base_url+'/user/assign_panel_coordinators',
                    ...
                    success: function(response, status ) {
                        ...
                        if(response.status == "200"){
                            ...
                            window.location.href = base_url+'/user/panel_coordinators/'+response.data;
                        }else{
                            Swal.fire(
                                'error',
                                response.reason,
...
//
    $('.panelCoordinatorList').on('click','.editAuthorListedBtn', function(){
    ...
    $('.panelCoordinatorList').on('click', '.removePaperAuthor', function(){
    ...
    $('#savePanelCoordinators').on('click', function(){
            $.ajax({
                ...
                success: function (response, status) {
                    if (response.status == "200") {
                        ...
                        swal.fire({
                            ...
                            if(result.isConfirmed){
                                window.location.href = base_url+'/user/panel_submission_menu/'+paper_id;
                            }
                        });
                    }
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == '200') {
                        window.location.href = base_url + '/user/panel_submission_menu/' + response.data.insert_id + '/new';
                    }else{
...
function getPanelCoordinators(paper_id = null){
    $('.panelCoordinatorList').html('');
    $.post(base_url+'/user/get_paper_authors',
        ...
        function(response){
            ...
            $.each(response.data, function(index, val){
                $('.panelCoordinatorList').append('<tr class="author_order" author_id="'+val.author_id+'" order="'+val.author_order+'" name="'+val.name+'" paper_authors_id="'+val.ID+'" paper_id="'+val.paper_id+'">'+
